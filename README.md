...
- ```control```  : Use this for budget control. This requires config key ```limit``` (that sets transaction limit per day) and ```cut``` (percentage of the transaction limit you wish to save into a seperate bucket). You can execute ```control show``` to show tables of each day or ```control graph``` that shows a plot of data in respect to time. So what is the data? It includes exceed (```show``` only, it shows true/false whether you have exceeded limit), saved (```show``` only, it shows ture/false whether you have a cut that is > 0), delta (the actual difference between limit and actual), and extra (money you save other than the required cut)
...
- ```--candlestick``` for ```graph time``` to show the nominal or total value as candlesticks.
